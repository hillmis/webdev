class FileManager {
    ...
    finishInlineRename(input) {
        ...
        if (newName && originalPath && newName !== originalPath.split('/').pop()) {
            ...
            // 检查是否是文件夹
            const isFolder = item && item.querySelector('.folder-icon');

            // 如果是文件，更新文件内容引用
            if (!isFolder && this.files.has(originalPath)) {
                const content = this.files.get(originalPath);
                this.files.delete(originalPath);
                this.files.set(newPath, content);

                // 如果这是当前选中的文件，更新引用
                if (this.selectedFile === originalPath) {
                    this.selectedFile = newPath;

                    // 更新打开的标签页（如果文件已打开）
                    const event = new CustomEvent('file-renamed', {
                        detail: { oldPath: originalPath, newPath: newPath }
                    });
            ...
            // 如果是文件夹，更新所有内部文件
                ...
                const oldPrefix = originalPath + '/';
                const newPrefix = newPath + '/';

                // 获取所有以旧文件夹路径开头的文件
                const filesToUpdate = Array.from(this.files.keys())
                    .filter(path => path.startsWith(oldPrefix));

                // 更新每个文件路径
                filesToUpdate.forEach(oldFilePath => {
                    const content = this.files.get(oldFilePath);
                    const newFilePath = newPrefix + oldFilePath.substring(oldPrefix.length);

                    this.files.delete(oldFilePath);
                    this.files.set(newFilePath, content);

                    // 如果这是当前选中的文件，更新引用
                    if (this.selectedFile === oldFilePath) {
                        this.selectedFile = newFilePath;

                        // 更新打开的标签页（如果文件已打开）
                        const event = new CustomEvent('file-renamed', {
                            detail: { oldPath: oldFilePath, newPath: newFilePath }
                        });
                        window.dispatchEvent(event);
                    }
                });

                // 更新文件夹内容容器的 data-parent 属性
                const folderContent = item.nextElementSibling;
                if (folderContent && folderContent.classList.contains('folder-content')) {
                    folderContent.dataset.parent = newPath;

                    // 更新所有子项目的路径
                    const childItems = folderContent.querySelectorAll('.file-tree-item');
                    childItems.forEach(childItem => {
                        const childPath = childItem.dataset.path;
                        const newChildPath = childPath.replace(oldPrefix, newPrefix);
                        childItem.dataset.path = newChildPath;
                    });
                }
            }
    ...
    refreshFileExplorer() {
        ...
        // 显示通知
        if (window.app) {
            window.app.showNotification('文件管理器已刷新');
        }
    }
